...
//         // Reset user's stats for this level
//         userSlot.currentActiveSlot = 0;
//         stats.levelReferrals[level] = 0;
//         emit SlotPositionUpdated(user, level, 0, 0);

//         // Process upgrade to next level
//         // _upgradeProcessSlotPurchase(user, level + 1);
...
//         if (referrer != address(0)) {
//             // Check if referrer has upgraded to this level before distributing rewards
//             if (!hasUpgraded[referrer][level]) {
//                 // If referrer hasn't upgraded, send rewards to mother wallet
//                 _sendReward(motherWallet, directReward, "pending_upgrade");
//                 emit RewardDistributed(
//                     motherWallet,
//                     user,
...
//                 );
//             } else {
//                 // Special handling for 11th referral at this level
...
//         // Rest of the generation pool distribution remains the same...
//         uint256 generationPool = (amount * 45) / 100;
//         address current = referrer;
//         uint256 unclaimedReward = 0;
...
//             if (upline == address(0) || !hasUpgraded[current][level]) {
//                 unclaimedReward = levelReward;
//                 continue;
//             }
...
//         // Last slot purchaser reward handling remains the same...
//         uint256 motherReward = (amount * 5) / 100;
//         address previousSlotUser = lastSlotPurchaser[level];
